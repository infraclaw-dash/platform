...
struct CoreRPCClient {
    ...
    /// `getmempoolentry` fields the integration framework consumes.
    /// dashd serializes the `instantlock` flag as a JSON *string*
    /// (`"true"` / `"false"`) rather than a bool — match the wire
    /// shape exactly or decoding throws.
    struct MempoolEntry: Decodable {
        let instantlock: String
        let unbroadcast: Bool
    }

    func getMempoolEntry(_ txid: String) async throws -> MempoolEntry {
        try await call("getmempoolentry", params: [.string(txid)])
    }
